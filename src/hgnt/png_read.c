...
/* read a png file. 
  ...
  ret <0 error 
*/
int png_read(const char *file_name,image *im)
{
   FILE *fp;
   png_structp png_ptr;
   png_infop info_ptr;
    ...
    */
   png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, (void *)user_error_ptr, user_error_fn, user_warning_fn);
...
/*	  
	  ...
	  if (!(im->supported_fmts & IM_MAPPED8))
		 png_set_expand(png_ptr);
	  else 
*/
	  {
...
/*
			// trans seem to be the corresponding alpha values
			if (info_ptr->trans) {  // is this ok === 
				im->backgroundIndex = info_ptr->trans[0];
				im->colorKeyIndex = info_ptr->trans[0];
				im->colorKey = 1;
			}
*/
		  }
